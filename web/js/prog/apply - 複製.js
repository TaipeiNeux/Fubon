...
function apply2_valid() {
    ...
    /*if (adultTag == 'N') { //若有radio顯示, 檢查是否有點選
...
function apply2(content) {
    ...
    adultHidden.val(applicantAdult);
...
function apply3_2(content) {
    ...
    $('#loansSum, #accordingToBillPlusOthers .input_f, #accordingToBill_publicExpense').on('blur', function() {
        ...
        if (isNaN(parseInt($thisValue.val()))) {
            $thisValue.val('');
        }
...
function apply5_1_1(content) {
    ...
    student_name.text(schoolName_isNational + ' ' + schoolName_name + ' ' + schoolName_isDay + '部');
    student_department.text(department);
    student_onTheJob.text((OnTheJob == 'N') ? "否" : "是");
    student_class.text(gradeId + '年' + classString);
...
function apply5_2(content) {
    ...
    if (loans == '1') {
        ...
        student_loansPrice.text(addDot(loansPrice));
        student_credit.text(addDot(loansSum) + "元");
        student_abroad.text(addDot(abroad) + "元");
        student_book.text(addDot(book) + "元");
        student_life.text(addDot(life) + "元");
        student_live.text(addDot(live) + "元");
        student_publicExpense.text(addDot(publicExpense) + "元");
        console.debug(student_publicExpense.length);
    } else if (loans == '2') {
        var loansPrice = content.freedom_sum;
        var FPA = content.freedom.FPA;
        ...
        student_live.text(addDot(live) + "元");
        student_publicExpense.text(addDot(publicExpense) + "元");
        student_FPA.text(addDot(FPA) + "元");
        student_credit.text(addDot(credit) + "元");
        student_music.text(addDot(music) + "元");
        student_loansPrice.text(addDot(loansPrice));
        student_practice.text(addDot(practice) + "元");
    }
}
