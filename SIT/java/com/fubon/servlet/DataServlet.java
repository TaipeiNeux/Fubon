...
import com.fubon.htmltopdf.Htmltopdf;
...
/**
 * Created with IntelliJ IDEA. User: Titan Date: 2016/4/25 Time: 上午 3:55 To
 * change this template use File | Settings | File Templates.
 */
public class DataServlet extends HttpServlet {

	private static final String[] FamilyInfoColumns = new String[] { "Name", "IdNo", "Birthday", "TelNo1", "TelNo2",
			"CellPhoneNo", "Zip", "Addr_2", "Addr_3", "Addr_5", "Addr_6", "Addr_7", "Addr_8", "Addr_9", "Addr_10",
			"Addr_11", "Addr_12", "Addr_13", "Addr_14", "Addr_15", "Addr_16", "Addr_17", "Addr_18", "Addr_19",
			"Addr_20", "Addr_21", "Addr_22", "Addr_23", "Addr_24", "Addr_25" };
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		this.doPost(req, resp); // To change body of overridden methods use File
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (req.getContentType() != null && req.getContentType().indexOf("multipart/form-data") != -1) {
			...
			// 設定上傳暫存目錄
			String uploadTempFolder = PropertiesUtil.loadPropertiesByClassPath("/config.properties")
					.getProperty("uploadTempFolder");
		...
		// 更新跳出網路註記
			...
			try {
				DownloadPdf(queryStringInfo, resp);
			} catch (Exception e) {

				e.printStackTrace();
			}
		}
		else if ("PdfFileDownload".equalsIgnoreCase(action)) 
		{
			try {
				PdfFileDownload(queryStringInfo,resp);
			} catch (Exception e) {
	...
	public void getPersonalInfo(JSPQueryStringInfo queryStringInfo, HttpServletResponse response)
		...
		try {
			...
			if (loginUserBean == null) {
				...
				String isRecord = ProjUtils.isPayHistory(userId, dao) ? "Y" : "N", id = "", name = "", birthday = "",
						marryStatus = "", cellPhone = "", email = "",mobile="";
				String domicilePhoneRegionCode = "", domicilePhonePhone = "";
				String telePhoneRegionCode = "", telePhonePhone = "";
				String domicileAddressAddress = "";
				String teleAddressAddress = "",teleAddressZipCode = "";
				...
				if ("Y".equalsIgnoreCase(isRecord)) {
					...
					if (!"sit".equalsIgnoreCase(env)) {
						...
						// teleAddressAddress =
						// ProjUtils.toAddress(aplyMemberHistoryData,"Aply","2");
						...
						//domicilePhoneRegionCode="22037966";
					...
					if (StringUtils.isNotEmpty(domicileAddressZipCode)) {
						...
						domicileAddressAddress = ProjUtils.toCityName(domicileAddressCityId, dao)
								+ ProjUtils.toZipCodeName(domicileAddressZipCode, dao) + domicileAddressLiner
								+ domicileAddressNeighborhood + "鄰" + domicileAddressAddress;
					}

					// domicileAddressAddress =
					// ProjUtils.toAddress(aplyMemberHistoryData,"Aply","1");
				}
				// 要帶出完整戶藉/通訊地址
				else {
                ...
                if (StringUtils.isNotEmpty(teleAddressZipCode)) {
                    ...
                    teleAddressAddress = ProjUtils.toCityName(teleAddressCityId, dao)
                            + ProjUtils.toZipCodeName(teleAddressZipCode, dao) + teleAddressAddress;
                }
				

				// 裝值到content
				content.put("notice_text",
						"<h3> 注意事項</h3><ol> <li>不開放線上修改之個人資料，您可透過以下兩種方式辦理：<br> (1)透過客服:可撥打客戶服務專線02-8751-6665按5由專人為您服務<br> (2)透過臨櫃:請您本人攜帶身分證正本及原留印鑑，至本行各<a href='123.html'><u>服務據點</u></a>辦理個人基本資料變更。<br></li> <li> 為符合「金融監督管理委員會指定非公務機關個人資料檔案安全維護辦法」之規定，本行就學貸款服務專區內，涉及<br>個人資料之交易，部分資料將以遮蔽之方式進行保護，若導致您無法確認資料之正確性，請您至本行櫃檯辦理或洽<br>24HR客服中心02-8751-6665按5將有專人竭誠為您服務。 </li></ol>");
	...
	public void showOTP(JSPQueryStringInfo queryStringInfo, HttpServletResponse response)
			throws ServletException, IOException {
		...
		while (true) {
			...
			if (!filterFontName.contains(font.getName()))
				break;
		}
	...
	public void saveDeferment0(JSPQueryStringInfo queryStringInfo, HttpServletResponse resp) {
		...
		try {
			...
			String hasData = "Y".equals(isLogin)
					? (ProjUtils.getNewsAplyMemberTuitionLoanHistoryData(loginUserBean.getUserId(),
							DaoFactory.getDefaultDao()) == null ? "N" : "Y")
					: "N";// 有無撥款紀錄
			...
			if ("Y".equalsIgnoreCase(isLogin)) {
				...
				SQLCommand update = new SQLCommand(
						"delete from Deferment_Doc where AplyIdNo = ? and (FlowLogId is null or FlowLogId = 0)");
				
				//update.
				update.addParamValue(userId);
				DaoFactory.getDefaultDao().queryByCommand(null, update,
						new QueryConfig().setExecuteType(QueryConfig.EXECUTE), null);

				for (String acnoSl : acnoSlList) {

					if (StringUtils.isEmpty(acnoSl))
						continue;
	...
	public void uploadDefermentDocument(JSPQueryStringInfo queryStringInfo, HttpServletResponse resp) {
		...
		try {
			...
			for (String inputName : uploadFileMap.keySet()) {
				...
				// additionalFile：在學證明
				String docType = "";
				if (inputName.startsWith("isPositiveFile"))
					docType = "1";
				else if (inputName.startsWith("isNegativeFile"))
					docType = "2";
				else if (inputName.startsWith("studentIdPositiveFile"))
					docType = "3";
				else if (inputName.startsWith("studentIdNegativeFile"))
					docType = "4";
				else if (inputName.startsWith("additionalFile"))
					docType = "5";

				// if("isPositiveFile".equalsIgnoreCase(inputName)) docType =
				// "1";
				// else if("isNegativeFile".equalsIgnoreCase(inputName)) docType
				// = "2";
				// else if("studentIdPositiveFile".equalsIgnoreCase(inputName))
				// docType = "3";
				// else if("studentIdNegativeFile".equalsIgnoreCase(inputName))
				// docType = "4";
				// else if("additionalFile".equalsIgnoreCase(inputName)) docType
				// = "5";
				...
				if (StringUtils.isNotEmpty(docType)) {
					...
					if (StringUtils.isNotEmpty(docId)) {
						...
						// SQLCommand delete = new SQLCommand("delete from
						// Deferment_Doc where AplyIdNo = ? and DocType = ? and
						// (FlowLogId is null or FlowLogId = 0)");
						// delete.addParamValue(userId);
						// delete.addParamValue(docType);
						// dao.queryByCommand(null,delete,new
						// QueryConfig().setExecuteType(QueryConfig.EXECUTE),null);
					}
					...
					Deferment_Doc.setValue("CreateTime",
					...
					try {
						...
						PreparedStatement ps = conn
								.prepareStatement("update Deferment_Doc set DocFile = ? where DocId = ?");
	...
	public void uploadApplyDocument(JSPQueryStringInfo queryStringInfo, HttpServletResponse resp) {
		...
		try {
			...
			for (String inputName : uploadFileMap.keySet()) {
				...
				// registerFile：註冊
				String docType = "";
				if (inputName.startsWith("idPositiveFile"))
					docType = "1";
				else if (inputName.startsWith("idNegativeFile"))
					docType = "2";
				else if (inputName.startsWith("registerFile"))
					docType = "3";
				else if (inputName.startsWith("lowIncomeFile"))
					docType = "4";
				...
				if (StringUtils.isNotEmpty(docType)) {
					...
					if (StringUtils.isNotEmpty(docId)) {
						...
						dao.queryByCommand(null, delete, new QueryConfig().setExecuteType(QueryConfig.EXECUTE), null);
						// SQLCommand delete = new SQLCommand("delete from
						// AplyMemberTuitionLoanDtl_Doc where DocId = ? and
						// DocType = ?");
						// delete.addParamValue(userId);
						// delete.addParamValue(docType);
						// dao.queryByCommand(null,delete,new
					...
					// 再新增
					DataObject aplyMemberTuitionLoanDtlDoc = DaoFactory
							.getDefaultDataObject("AplyMemberTuitionLoanDtl_Doc");
					...
					aplyMemberTuitionLoanDtlDoc.setValue("CreateTime",
							DateUtil.convert14ToDate("yyyy-MM-dd HH:mm:ss", DateUtil.getTodayString()));
					...
					try {
						...
						PreparedStatement ps = conn.prepareStatement(
								"update AplyMemberTuitionLoanDtl_Doc set DocFile = ? where DocId = ?");
		...
		try {
			...
			if (branchRS.size() != 0) {
				...
				for (String day : noBusinessDays) {
					//System.out.println("@@@@@"+noBusinessDays);
					noBusiness.put(day);
				...
				for (String expectDate : expectMap.keySet()) {
					...
					List<String> timeList = expectMap.get(expectDate);
					// String isFull = timeList.size() == periodPeoples ? "Y" :
					// "N";
					...
					for (String time : timeMap.keySet()) {
						...
						if ("N".equalsIgnoreCase(timeFull))
							isFull = "N";
	...
	public void getFamilyInfo(JSPQueryStringInfo queryStringInfo, HttpServletResponse resp) {
        ...
        try{
            ...
            if(loginUserBean == null) {
                ...
                DataObject aplyMemberDataFamilyCheck = null;
                aplyMemberDataFamilyCheck= ProjUtils.getNewsAplyMemberTuitionLoanHistoryData(userId,dao);
                if(aplyMemberDataFamilyCheck==null)
                	aplyMemberDataFamilyCheck=ProjUtils.getNewsAplyMemberTuitionLoanHistoryData(userId,dao);
                ...
                if(StringUtils.isNotEmpty(draftXML)) {
                    ...
                    else if (aplyMemberDataFamilyCheck != null)
                    {
                    	String dbColumnPrefix = familyTypeMap.get(type);
                    	String zipCode = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix + "_Zip");
                        String cityId = ProjUtils.toCityId(zipCode,dao);
                       
                        
                        id = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix + "_IdNo");      
                     
                        name = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix + "_Name");

                        birthday = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix + "_Birthday");
                        birthday = ProjUtils.toBirthday(birthday);

                        telePhoneRegionCode = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix + "_TelNo1");
                        telePhonePhone = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix + "_TelNo2");
                        mobile = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix + "_CellPhoneNo");

                        domicileAddressCityId = cityId;
                        domicileAddressZipCode = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix + "_Zip");
                        domicileLinerName = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix + "Village");
                        domicileAddressNeighborhood = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix + "_Addr_3");
                        domicileAddressAddress = aplyMemberDataFamilyCheck.getValue(dbColumnPrefix+"Addr");
...
//                    domicileLinerName = ProjUtils.toAddressAllMark(domicileLinerName);
	...
	public void getQA(JSPQueryStringInfo queryStringInfo, HttpServletResponse resp) {
		...
		try {
			...
			for (DataObject d : ret) {
				...
				SQLCommand queryDetail = new SQLCommand(
						"select DataNo,Question,Answer from QA_Data\n" + "where TopicNo = ?\n" + "order by DataNo");
	...
	public void getDocument(JSPQueryStringInfo queryStringInfo, HttpServletResponse resp) {
		...
		try {
			...
			for (DataObject d : ret) {
				...
				SQLCommand queryDetail = new SQLCommand(
						"select DataNo,Title from Document_Data\n" + "where TopicNo = ?\n" + "order by DataNo");
				queryDetail.addParamValue(topicNo);
				Vector<DataObject> detailRS = new Vector<DataObject>();
				dao.queryByCommand(detailRS, queryDetail, null, null);
	...
	public void downloadDefermentDocument(JSPQueryStringInfo queryStringInfo, HttpServletResponse resp) {
		...
		try {
			...
			PreparedStatement ps = conn
					.prepareStatement("select original_file_name,DocFile from Deferment_Doc where DocId = ?");
			...
			while (rs.next()) {
				...
				JSPUtils.downLoadFile(resp, getServletContext().getMimeType(fileName), f.getPath(),
						!"Y".equalsIgnoreCase(isPreview), f.getName(), isIE);
	...
	public void downloadApplyDocument(JSPQueryStringInfo queryStringInfo, HttpServletResponse resp) {
		...
		try {
			...
			PreparedStatement ps = conn.prepareStatement(
					"select original_file_name,DocFile from AplyMemberTuitionLoanDtl_Doc where DocId = ?");
			ps.setString(1, docId);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				String uploadTempFolder = PropertiesUtil.loadPropertiesByClassPath("/config.properties")
	...
	public void downloadDocument(JSPQueryStringInfo queryStringInfo, HttpServletResponse resp) {
		...
		try {
			...
			while (rs.next()) {
				...
				JSPUtils.downLoadFile(resp, getServletContext().getMimeType(".pdf"), f.getPath(),
						!"Y".equalsIgnoreCase(isPreview), f.getName(), isIE);
	...
	public void repaymentInquiry(JSPQueryStringInfo queryStringInfo, HttpServletResponse resp) {
		...
		try {
			...
			columnMap.put("TOT_AMT", new String[] { "還款金額", "Y", "Y" });
			...
			ProjUtils.saveLog(DaoFactory.getDefaultDao(), queryStringInfo.getRequest(), getClass().getName(),
					"repaymentInquiry", result);
		}
		...
		try {
			...
			if (applyIsPromo != null) {
				...
				if (!dao.querySingle(applyIsPromo, null)) {
					applyIsPromo.setValue("CreateTime",
							DateUtil.convert14ToDate("yyyy-MM-dd HH:mm:ss", DateUtil.getTodayString()));
...
//                total += setMyLoanAccountDetail(doc,client_detail,isDelay,acnoSl,bankName);
	...
	public static int setMyLoanAccountDetail(Document doc, JSONArray client_detail, boolean isDelay, String acnoSl,
		...
		if (detailArray.length() != 0) {
			...
			clientDetail.put("pay_day", (StringUtils.isNotEmpty(nextIntDate) && nextIntDate.length() < 2) ? ""
					: nextIntDate.substring(nextIntDate.length() - 2, nextIntDate.length()));
	...
	public void DownloadPdf(JSPQueryStringInfo queryStringInfo, HttpServletResponse response) throws Exception {
		LoginUserBean loginUserBean = ProjUtils.getLoginBean(queryStringInfo.getRequest().getSession());
		String userId = loginUserBean.getUserId();
		String userName = loginUserBean.getUserName();
		String filename = userName + "的就學貸款申請撥款通知書.pdf";
		filename = new String(filename.getBytes(), "ISO8859-1");
		Htmltopdf me = new Htmltopdf(userId);

		File file = new File("C:\\" + userId + ".pdf");
	...
	public void PdfFileDownload(JSPQueryStringInfo queryStringInfo, HttpServletResponse response) throws Exception 
	{
		LoginUserBean loginUserBean = ProjUtils.getLoginBean(queryStringInfo.getRequest().getSession());
		String userId = loginUserBean.getUserId();
		String pdfpath = PropertiesUtil.loadPropertiesByClassPath("/config.properties")
				.getProperty("pdfpath");
		
		String filename =  userId+".pdf";
		

		File file = new File(pdfpath + userId + ".pdf");
