        ...
        success: function(html) {
            ...
            nextBtn.find('.next').off('click').on('click', function(ev) {
                ...
                if (isValid) {
					...
					//2016-07-16 added by titan,因為像關係人是把該欄整個display:none，所以在送出前要將不可視的元素拿掉，以免多傳了相關的資訊
					if(Content.flow.flowId == 'apply' && stepId == 'apply2') {
						
						var parents = form.find('.parents:hidden');
						parents.remove();
					}
