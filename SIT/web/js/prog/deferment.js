...
function deferment_2(content) {
    ...
    if (RadioClicked == 'Y') {
        $('#registerStamp_y').trigger('click');
    } else if (RadioClicked == 'N') {
        $('#registerStamp_n').trigger('click');
    }
}
...
function uploadEvent(input) {
    ...
    defaultFileArray.off('change').on('change', function(ev) {
		...
		var fileTR = fileBtn.parent();
		
		$.each(fileTR, function(index, value){
			console.debug(value);
			var ele = $(value);
			if(ele.is(':visible')){
				uploadNumber++;
		...
		console.debug(uploadNumber);
        
        if (uploadNumber == 10 && inputText != '修改檔案') {
            $('#documentNumber').show();
        } else {
            $('#documentNumber').hide();
